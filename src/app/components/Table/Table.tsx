...
type TableProps<Row extends BaseRow, Column extends BaseColumn = BaseColumn> = {
    ...
    defaultSortColumn?: keyof Column;
};

type SortState<Column extends BaseColumn> = {
    column: keyof Column;
    direction: 'asc' | 'desc';
};
...
export default function Table<
    Row extends BaseRow,
    Column extends BaseColumn = BaseColumn,
>({
    data,
    columns,
    onRenderRow,
    defaultSortColumn = 'team',
    ...
    const sortedData = useMemo(() => {
        ...
        return [...data].sort((a, b) => {
            ...
            const aVal = rowA[sort.column as string];
            const bVal = rowB[sort.column as string];
    ...
    const handleSort = (column: TableColumn<Column>) => {
        ...
        setSort((prev) => ({
            ...
            direction:
                prev.column === column.sortKey && prev.direction === 'asc'
                    ? 'desc'
                    : 'asc',
        }));
    };
        ...
        return (
            <th
                ...
                className={`border border-gray-300 px-4 py-2 text-left ${
                    column.sortKey ? 'cursor-pointer hover:bg-gray-200' : ''
                }`}
            >
                ...
                <thead className="bg-gray-100">
                    <tr>
                        {columns.map((column, index) =>
                            renderHeaderWithSort(column, index),
                        )}
                    </tr>
                </thead>
                <tbody>{sortedData.map(onRenderRow)}</tbody>
            </table>
        </div>
    );
}
